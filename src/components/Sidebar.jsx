...
const Sidebar = () => {
  ...
  useEffect(() =>{
    if(dToken){
      navigate('/doctor-dashboard'); // Redirect to doctor dashboard if doctor is logged in
    }else if(aToken){
      navigate('/admin-dashboard'); // Redirect to admin dashboard if admin is logged in
    }
  }, [aToken, dToken])
